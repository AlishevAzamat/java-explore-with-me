...
public interface StatRepository extends JpaRepository<Hit, Long> {
    @Query("select new ru.practicum.stats.service.model.Stats(app, uri, count(uri) as hits) from Hit " +
            "where uri in (?1) " +
    ...
    @Query("select new ru.practicum.stats.service.model.Stats(app, uri, count(distinct uri) as hits) from Hit " +
            "where uri in (?1) " +
            "and timestamp >= ?2 and timestamp <= ?3 " +
